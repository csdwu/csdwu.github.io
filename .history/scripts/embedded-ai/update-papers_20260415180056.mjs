...
const TENCENT_HUNYUAN_API_KEY =
  process.env.TENCENT_HUNYUAN_API_KEY || process.env.HUNYUAN_API_KEY || "";
const TENCENT_HUNYUAN_MODEL =
  process.env.TENCENT_HUNYUAN_MODEL || "hunyuan-turbos-latest";
const TENCENT_HUNYUAN_BASE_URL =
...
/**
 * Strict AND-based search.
 * TinyML mirrors the awesome-tinyml intent but tightened.
 * Embedded AI uses strict AND queries, then CCF-A venue filtering.
 */
...
/**
 * Curated CCF-A venues most relevant to Embedded AI.
 * Match against arXiv journal_ref / comment / venue metadata.
 */
...
async function fetchArxivEntriesForQuery(sourceDef, query) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    ...
    const response = await fetch(url.toString(), {
      headers: {
        "User-Agent": "embedded-ai-page-updater/1.0 (contact: guo.qilong.self@gmail.com)",
...
async function classifyWithTencentHunyuan(paper) {
  if (!TENCENT_HUNYUAN_API_KEY) {
    throw new Error("Missing TENCENT_HUNYUAN_API_KEY for Tencent Hunyuan fallback.");
  }
  ...
  const response = await fetch(TENCENT_HUNYUAN_BASE_URL, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
      Authorization: `Bearer ${TENCENT_HUNYUAN_API_KEY}`,
    },
    body: JSON.stringify({
  ...
  if (!response.ok) {
    const text = await response.text();
    throw new Error(
      `Tencent Hunyuan request failed: ${response.status} ${response.statusText} :: ${text}`
    );
  }
  ...
  if (!content) {
    throw new Error("Tencent Hunyuan returned no message content.");
  }
...
async function classifyWithFallback(ai, paper) {
  ...
  if (OPENROUTER_API_KEY) {
    try {
      ...
      console.warn(
        `[fallback] OpenRouter failed for "${paper.title}". Trying Tencent Hunyuan next. ${error.message}`
      );
    }
  }
  ...
  if (TENCENT_HUNYUAN_API_KEY) {
    try {
      return await classifyWithTencentHunyuan(paper);
    } catch (error) {
      errors.push(`Tencent Hunyuan: ${error.message}`);
    }
  }
...
async function main() {
  if (!GEMINI_API_KEY && !OPENROUTER_API_KEY && !TENCENT_HUNYUAN_API_KEY) {
    throw new Error(
      "At least one of GEMINI_API_KEY, OPENROUTER_API_KEY, or TENCENT_HUNYUAN_API_KEY must be set."
    );
  }
  ...
  // Re-apply current filter rules so old stale results don't remain forever.
